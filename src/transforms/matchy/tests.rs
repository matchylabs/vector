...
use crate::{
    ...
    transforms::{
        ...
        matchy::{
            config::{DatabaseConfig, ExtractionConfig, MatchyConfig},
        },
    },
};
...
fn create_test_database() -> tempfile::NamedTempFile {
    ...
    data.insert("threat".to_string(), matchy::DataValue::String("known_bad".to_string()));
...
fn remap_with_extraction() {
    ...
    let mut transform = MatchyTransform::new(config).unwrap();
    let event = Event::Log(LogEvent::from("Traffic to evil.com and example.com detected"));
